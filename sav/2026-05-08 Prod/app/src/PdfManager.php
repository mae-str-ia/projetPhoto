...
class PdfManager {
    ...
    private static function convertViaPdftoppm($pdftoppm, $pageIndex, $outputPath) {
        ...
        $cmd = '"' . $pdftoppm . '"'
            ...
            . ' -r 150'
            . ' -png'
            . ' ' . escapeshellarg(str_replace('/', '\\', self::$pdf_file))
            . ' ' . escapeshellarg(str_replace('/', '\\', $prefix))
            . ' 2>&1';
    ...
    private static function convertViaGhostscript($gs, $pageIndex, $outputPath) {
        ...
        $cmd = '"' . $gs . '"'
            ...
            . ' -dFirstPage=' . $pageNum
            . ' -dLastPage=' . $pageNum
            . ' -sOutputFile=' . escapeshellarg(str_replace('/', '\\', $outputPath))
            . ' ' . escapeshellarg(str_replace('/', '\\', self::$pdf_file))
            . ' 2>&1';
    ...
    /**
     * Find pdftoppm executable (Poppler, available via Calibre)
     */
    private static function findPdftoppm() {
        ...
        foreach ($candidates as $cmd) {
            if (file_exists($cmd) || !empty(shell_exec('where ' . escapeshellarg($cmd) . ' 2>nul'))) {
                return $cmd;
            }
        }
    ...
    private static function findGhostscript() {
        ...
        foreach ($candidates as $cmd) {
            $check = trim(shell_exec('where ' . escapeshellarg($cmd) . ' 2>nul') ?? '');
            if ($check) {
                return $check;
            }
        }
