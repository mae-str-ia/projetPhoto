...
const PageEditor = {
    ...
    _leftPageMarginsPercent() {
        ...
        return {
            ...
            right: Math.max(0, Math.min(90, (parseFloat(margins.rightCm ?? 3) / width) * 100)),
    ...
    _slotInMargins(slot) {
        const m = this._leftPageMarginsPercent();
    ...
    _adjacentPhotoPage(offset) {
        const total = typeof TOTAL_PAGES_JS !== 'undefined' ? TOTAL_PAGES_JS : BOOK_CONFIG.totalPages;
        const target = this.pageNumber + offset;
        if (target < 2 || target > total) return null;
        return target;
    },
