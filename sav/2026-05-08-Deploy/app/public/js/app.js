...
const App = {
    ...
    async generateFinalPdf() {
        if (!confirm('Générer le PDF complet (livre.print.pdf) ?\n\nCela va:\n1. Capturer les pages photos (~12 min)\n2. Fusionner avec le PDF texte\n3. Corriger les dimensions\n\nCela peut prendre environ 15-20 minutes...')) {
            return;
        }

        const loading = document.createElement('div');
        loading.className = 'loading';
        loading.innerHTML = '<div style="text-align: center;"><div>Génération du PDF complet en cours...</div><div style="font-size: 0.9em; margin-top: 0.5em; color: #666;">Cela peut prendre 15-20 minutes. Ne fermez pas cette page.</div></div>';
        document.body.appendChild(loading);
        ...
        try {
            ...
            this.notify('✅ PDF complet généré avec succès! (livre.print.pdf)', 'success', 5000);
        } catch (err) {
            loading.remove();
            alert('Erreur: ' + err.message);
        }
