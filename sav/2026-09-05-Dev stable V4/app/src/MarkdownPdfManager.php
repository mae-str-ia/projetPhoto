...
class MarkdownPdfManager {
    ...
    private static function writeBuildMarkdown($manualToc = null, $emitSectionMetadata = false) {
        ...
        // Replace spaces with non-breaking spaces for French typography
        // After opening guillemets and before closing guillemets
        $markdown = str_replace(['« ', ' »', ' !', ' ?', ' :', ' ;'], ["«\u{00A0}", "\u{00A0}»", "\u{00A0}!", "\u{00A0}?", "\u{00A0}:", "\u{00A0};"], $markdown);
        ...
        // Define margins for sections based on layout type
        $markdown = preg_replace_callback(
            '/<!--\s*section:\s*([^>]+)\s*-->/u',
            function($matches) {
                $fullMatch = $matches[0];

                if (strpos($fullMatch, 'layout=text-both') !== false) {
                    // For text-both sections, use inside/outside (alternating)
                    if (strpos($fullMatch, 'mode=inverse') !== false) {
                        $typstCode = "#set page(margin: (inside: 2cm, outside: 3.5cm, top: 2cm, bottom: 2cm))";
                    } else {
                        $typstCode = "#set page(margin: (inside: 3.5cm, outside: 2cm, top: 2cm, bottom: 2cm))";
                    }
                } else {
                    $typstCode = "#set page(margin: (left: 3.5cm, right: 2cm, top: 2cm, bottom: 2cm))";
                }

                return self::rawTypstBlock($typstCode) . "\n\n" . $fullMatch;
            },
            $markdown
        );
        ...
        $markdown = preg_replace_callback(
            ...
            function ($matches) use ($manualToc, $emitSectionMetadata) {
                ...
                if ($manualToc === null) {
                    $typst[] = '#outline(title: [Table des matières], depth: 2)';
        ...
        // Replace <!-- page-break --> with marker for later conversion to #pagebreak()
        $markdown = str_replace('<!-- page-break -->', '[PAGEBREAK]', $markdown);
        ...
        $markdown = self::injectFootnoteAnchorMarkers($markdown);
    ...
    private static function injectFootnoteAnchorMarkers($markdown) {
        // Add anchors only before footnote references (not definitions)
        // This allows us to track where references are located

        // Replace [^N] with <ref-N>[^N]  (only in text, not in definitions)
        // Use negative lookbehind to avoid matching definitions [^N]:
        $markdown = preg_replace_callback(
            '/(?<!\n\[\^)(\[\^(\d+)\])(?!\s*:)/u',
            function($matches) {
                $num = $matches[2];
                return "<ref-$num>" . $matches[1];
            },
            $markdown
        );
    ...
    private static function sectionMetadataTypst($attrs) {
        ...
        $code = '#context metadata((kind: "section", id: "' . self::escapeTypstString($id) . '", layout: "' . self::escapeTypstString($layout) . '", page: counter(page).get().first()))';
    ...
    private static function rawTypstDirective($name) {
        $typst = [
            'blank-page' => "#set page(footer: none, foreground: none)\n#pagebreak()\n#set page(footer: outside-page-number(), foreground: page-foreground())",
...
TYPST;
    ...
    private static function restoreFootnoteAnchors($content) {
        // Add anchors <ref-N> and <note-N> for footnote verification (debug mode)
        // Only add anchors if DEBUG_NOTES is enabled

        if (!defined('DEBUG_NOTES') || !DEBUG_NOTES) {
            return $content;
        }
        ...
        $content = preg_replace_callback(
            ...
            function($matches) use (&$footnote_count) {
                $footnote_count++;
                // Use empty box with label for reference anchor (allows query to find page)
                return "#box(width:0pt)[]<ref-$footnote_count> #footnote[#box(width:0pt)[]<note-$footnote_count>";
            },
            $content
        );
    ...
    private static function styleDialogueAndCourierBlocks($content) {
        ...
        $content = preg_replace_callback(
            ...
            function($matches) {
                $blockContent = $matches[1];
                return '#block(width: 100%, inset: (left: 1em, right: 1em))[#set text(font: "Segoe Print", size: 9pt)' . $blockContent . "\n]";
            },
            $content
        );
    ...
    private static function writeTypstFile($emitHeadingMetadata = false, $sourceToFinalPageMap = null) {
        ...
        $binding = (float)($props['textBindingCm'] ?? 1.5);
        $showCropMarks = SHOW_CROP_MARKS;
        ...
        // Restore footnote anchor references that were lost by Pandoc
        $content = self::restoreFootnoteAnchors($content);
        ...
        $cropMarksCode = $showCropMarks ? <<<'CROPMARKS'
#let crop-marks = [
...
CROPMARKS
        ...
        $cropMarksUsage = $showCropMarks ? 'crop-marks' : '';
...
#let page-foreground() = context {
  running-page-header()
  crop-marks
}

#set page(width: 24cm, height: 16cm, margin: (inside: {$left}cm, outside: {$right}cm, top: {$top}cm, bottom: {$bottom}cm), header: [#counter(footnote).update(0)], footer: outside-page-number(), foreground: page-foreground())
#set text(font: "Libertinus Serif", size: 11pt, lang: "fr")
#set par(justify: true, leading: 0.85em, spacing: 1.5em)
#set footnote.entry(indent: 0em, clearance: 0.5em, gap: 0.5em)
...
TYPST;
        ...
        // Generate footnote verification report only in debug mode
        $footnotecheckReport = (defined('DEBUG_NOTES') && DEBUG_NOTES)
            ? self::generateFootnoteCheckCode($sourceToFinalPageMap)
            : '';

        $content = self::processPageBreakMarkers($content);

        if (file_put_contents(self::TYPST_FILE, $prelude . $content . "\n" . $footnotecheckReport) === false) {
            throw new Exception('Impossible d ecrire ' . self::TYPST_FILE);
        }
    }

    private static function processPageBreakMarkers($content) {
        // Pandoc escapes [PAGEBREAK] as \[PAGEBREAK\], fix this
        $content = str_replace('\\[PAGEBREAK\\]', '#pagebreak()', $content);
        // Also handle other variations
        $content = str_replace('[PAGEBREAK]', '#pagebreak()', $content);
        return $content;
    }

    private static function generateFootnoteCheckCode($sourceToFinalPageMap = null) {
        $pageMapTypst = self::pageMapTypst($sourceToFinalPageMap);

        return <<<FOOTNOTE_CHECK

#pagebreak()
#heading(outlined: false)[Vérification des notes de bas de page]
#v(0.5em)

#let check-footnote-pages() = context {
  let page-map = $pageMapTypst

  let convert-page(source-page) = {
    if source-page > 0 and source-page <= page-map.len() {
      page-map.at(source-page - 1)
    } else {
      source-page
    }
  }

  let footnote-numbers = range(1, 51)
  let issues = ()
  let total = 0

  // Collect reference and note pages
  for num in footnote-numbers {
    let ref-label = label("ref-" + str(num))
    let note-label = label("note-" + str(num))

    let ref-results = query(selector(ref-label))
    let note-results = query(selector(note-label))

    if ref-results.len() > 0 and note-results.len() > 0 {
      total += 1
      let ref-page = counter(page).at(ref-results.first().location()).first()
      let note-page = counter(page).at(note-results.first().location()).first()

      let ref-page-final = convert-page(ref-page)
      let note-page-final = convert-page(note-page)

      if ref-page-final != note-page-final {
        issues.push((num: num, ref-page: ref-page-final, note-page: note-page-final))
      }
    }
  }

  if issues.len() > 0 {
    text(size: 11pt, fill: red, weight: "bold")[
      ⚠ #(issues.len()) note(s) mal placée(s) sur #(total) :
    ]
    v(0.3em)

    for issue in issues {
      text(size: 10pt)[
        Note [^#(issue.num)] : référence page *#(issue.ref-page)* → note page *#(issue.note-page)*
      ]
    }
  } else if total > 0 {
    text(size: 11pt, fill: green, weight: "bold")[
      ✓ Toutes les #(total) notes sont correctement placées
    ]
  } else {
    text(size: 11pt)[Aucune référence de note trouvée.]
  }
}

#check-footnote-pages()
...
FOOTNOTE_CHECK;
    ...
    private static function queryBuildMetadata($typst) {
        ...
        foreach ($data as $item) {
            ...
            if (($value['kind'] ?? '') === 'heading') {
                ...
                if ($title === '' || $title === 'Table des matières') continue;
    ...
    private static function buildManualTocMarkdown($headings) {
        $lines = [
            self::rawTypstBlock('#heading(outlined: false)[Table des matières]'),
        ];
        foreach ($headings as $heading) {
