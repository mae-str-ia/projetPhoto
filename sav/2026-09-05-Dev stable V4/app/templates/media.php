...
$_book = BookManager::load();
$bookProperties = $_book['properties'] ?? BookManager::defaultProperties();
        ...
        <div class="media-info">
            ...
            <div class="media-usage">
                ...
                <?php else: ?>
                    ...
                    foreach ($m['usedOnPages'] as $pn):
                        $pn = intval($pn);
                    ?>
                        <a href="?page=editor&num=<?php echo $pn; ?>" class="media-page-link"><?php echo $pn + $offset; ?>–<?php echo $pn + 1 + $offset; ?></a>
                    <?php endforeach; ?>
                <?php endif; ?>
            </div>
        </div>
...
const mediaApp = {
    ...
    openPagePicker(btn) {
        ...
        for (let p = 2; p <= total - 1; p += 2) {
            ...
            opt.textContent = `Page ${p + offset}–${p + 1 + offset}`;
    ...
    refreshCardUsage(mediaId, addedPage) {
        ...
        if (!existing.includes(String(addedPage))) {
            ...
            a.textContent = `${addedPage + offset}–${addedPage + 1 + offset}`;
